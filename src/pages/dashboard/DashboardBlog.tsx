...
import { useDashboard, BlogPost } from "../../components/dashboard/DashboardContext";

const DashboardBlog = () => {
    const { blogPosts, addBlogPost, updateBlogPost, deleteBlogPost } = useDashboard();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (editingId) {
            ...
            addBlogPost(formData as Omit<BlogPost, "id" | "date" | "views" | "author">);
        }
        handleCloseModal();
    };
    ...
    return (
        <div className="space-y-6 animate-in fade-in duration-500">
            <div className="flex sm:items-center justify-between flex-col sm:flex-row gap-4">
                <div>
                    <h1 className="font-display text-3xl font-bold tracking-tight text-white">Blog Posts</h1>
                    <p className="text-muted-foreground text-sm">Create and manage your articles.</p>
                </div>
                <button
                    ...
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-white/5">
                            {filteredPosts.length === 0 ? (
                                <tr>
                                    <td colSpan={6} className="px-6 py-12 text-center text-muted-foreground">No blog posts found.</td>
                                </tr>
                            ) : (
                                filteredPosts.map((post) => (
                                    <tr key={post.id} className="hover:bg-white/[0.02] transition-colors group">
                                        ...
                                        <td className="px-6 py-4 text-muted-foreground">{post.views?.toLocaleString()}</td>
                                        <td className="px-6 py-4">
                                            <span className={`px-2.5 py-1 rounded-full text-xs font-medium border ${post.status === 'Published' ? 'bg-green-500/10 text-green-500 border-green-500/20' : 'bg-yellow-500/10 text-yellow-500 border-yellow-500/20'
                                                }`}>
                                                {post.status}
                                            </span>
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-right">
                                            <div className="flex items-center justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                                <button onClick={() => handleOpenModal(post)} className="p-1.5 text-muted-foreground hover:text-primary transition-colors rounded-md hover:bg-white/5" title="Edit">
                                                    <Edit2 size={16} />
                                                </button>
                                                <button onClick={() => handleDelete(post.id)} className="p-1.5 text-muted-foreground hover:text-red-500 transition-colors rounded-md hover:bg-red-500/10" title="Delete">
                                                    <Trash2 size={16} />
                                                </button>
                                            </div>
                                        </td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm animate-in fade-in">
                    <div className="bg-[#0A0C10] border border-white/10 rounded-2xl w-full max-w-xl shadow-2xl overflow-hidden animate-in zoom-in-95">
                        <div className="flex items-center justify-between p-6 border-b border-white/5">
                            <h2 className="text-xl font-display font-semibold text-white">{editingId ? 'Edit Post' : 'Compose Post'}</h2>
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-white/80 mb-1.5">Post Title</label>
                                <input
                                    required
                                    type="text"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-white/80 mb-1.5">Article Content (Markdown supported)</label>
                                <textarea
                                    required
                                    rows={10}
                                    value={formData.content}
                                    onChange={e => setFormData({ ...formData, content: e.target.value })}
                                    className="w-full bg-black border border-white/10 rounded-lg px-4 py-2.5 text-sm text-white focus:outline-none focus:border-primary/50 resize-y min-h-[300px]"
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex gap-4 items-start">
                                    ...
                                    <div className="flex-1 space-y-2">
                                        ...
                                        <p className="text-[10px] text-muted-foreground">URL or direct upload. Images saved in localStorage have size limits.</p>
                                    </div>
                                </div>
                            </div>
