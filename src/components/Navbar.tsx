...
import { Menu, X, ChevronDown, Globe, TrendingUp, Share2, Smartphone, Layers, ShoppingCart, ArrowUpRight, Phone, Instagram, Linkedin } from "lucide-react";
import { TikTokIcon, YoutubeIcon } from "@/components/icons/BrandIcons";

type NavLink = { label: string; href: string; isPage: boolean };

const socialLinks = [
  { label: "Instagram", href: "https://instagram.com/blesskimbi", icon: Instagram },
  { label: "LinkedIn", href: "https://www.linkedin.com/in/bless-kimbi-09413936a/", icon: Linkedin },
  { label: "TikTok", href: "https://www.tiktok.com/@blesskimbi", icon: TikTokIcon },
  { label: "YouTube", href: "https://www.youtube.com/@blesskimbi", icon: YoutubeIcon },
];
    ...
    <>
      ...
      >
        <div className="max-w-6xl mx-auto px-5 sm:px-8 h-[72px] flex items-center justify-between gap-4">
          ...
          <a
            href="tel:+237675126845"
            aria-label="Call +237 675 126 845"
            title="+237 675 126 845"
            className="hidden xl:inline-flex w-9 h-9 rounded-full bg-gold/10 hover:bg-gold/20 items-center justify-center text-gold transition-colors shrink-0"
          >
            <Phone size={15} />
          </a>
          ...
          <div className="hidden xl:flex items-center gap-2 shrink-0">
            {socialLinks.map(({ label, href, icon: Icon }) => (
              <a
                ...
                aria-label={label}
                className="w-8 h-8 rounded-full bg-muted hover:bg-primary hover:text-white flex items-center justify-center text-navy transition-colors duration-200"
              >
                <Icon size={14} />
              </a>
            ))}
            ...
            <div className="flex items-center justify-center gap-3 mt-8">
              {socialLinks.map(({ label, href, icon: Icon }) => (
                <a
                  ...
                  rel="noopener noreferrer"
                  aria-label={label}
                  className="w-10 h-10 rounded-full bg-muted hover:bg-primary hover:text-white flex items-center justify-center text-navy transition-colors duration-200"
                >
                  <Icon size={16} />
                </a>
              ))}
